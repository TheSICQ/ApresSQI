...
from ec import point_difference
...
def _points_range(rr, P, l, Q=None, PmQ=None):
    r"""
    ...
    if Q is None:
        ...
        R = xDBLMUL(1, a, Q, P, PmQ)
        yield R
        diff = xDBLMUL(1, (a-s) % l, Q, P, PmQ)
...
class FastEllipticPolynomial:
    ...
    The input can be an element of any algebra over the base ring::
        ...
        15*t + 19
    """
    def __init__(self, isom, E, n, P, l, Q=None, PmQ=None):
        #isom is necessary because the addition has to work on Montgomery curves
        #while the sqrt_velu code is for short weierstrass
        r"""
        ...
        I, J, K = IJK
        xI = (R.eval_isomorphism_x(isom) for R in _points_range(I, P, l, Q, PmQ))
        xJ = [R.eval_isomorphism_x(isom) for R in _points_range(J, P, l   )]
        xK = (R.eval_isomorphism_x(isom) for R in _points_range(K, P, l, Q, PmQ))
...
class EllipticCurveHom_velusqrt(EllipticCurveHom):
    ...
    .. SEEALSO::
        ...
        :class:`~sage.schemes.elliptic_curves.ell_curve_isogeny.EllipticCurveIsogeny`
    """
    def __init__(self, E, P, l, *, codomain=None, model='montgomery', Q=None):
        r"""
        ...
        self._EllipticCurveIsogeny__algorithm = 'velu_sqrt'
        self._degree = l
        ...
        # Generating a point outside the subgroup
        # For our case it will be sufficient to kill the l-torsion part
        F2 = E.base_field()
        assert F2.degree() == 2

        P_on_twist = P.on_twist()
        Q = None
        lpow = l**2 #should be sufficient in all cases (big powers are not that common)
        while not Q:
            Q = xPoint(F2.random_element(), E)
            while Q.on_twist() != P_on_twist:
                Q = xPoint(F2.random_element(), E)
            Q = Q.xMUL(lpow)

        PmQ = xPoint(point_difference(P, Q, x_only=True), E)
        self._Q = Q
        self._PmQ = PmQ

        self._internal_base_ring = E.base_ring()

        self._h0 = FastEllipticPolynomial(self._pre_iso, self._raw_domain, self._degree, self._P, l)
        self._h1 = FastEllipticPolynomial(self._pre_iso, self._raw_domain, self._degree, self._P, l, self._Q, self._PmQ)
